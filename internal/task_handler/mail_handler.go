...
func (m *MailHandler) Handle(ctx context.Context, task *model.Task) *taskerrors.TaskError {
	var dataPayload model.SendMailTemplate

	// 1. Decode payload

	if err := json.Unmarshal(task.Payload, &dataPayload); err != nil {
		return taskerrors.NonRetryable(fmt.Errorf("invalid send-mail payload: %w", err))
	}
	...
	if err != nil {
		return taskerrors.NonRetryable(fmt.Errorf("no such template present: %w", err))
	}

	// 3. Render template with data
	var body bytes.Buffer
	if err := tmpl.Execute(&body, dataPayload.Data); err != nil {
		return taskerrors.NonRetryable(fmt.Errorf("template render failed: %w", err))
	}
